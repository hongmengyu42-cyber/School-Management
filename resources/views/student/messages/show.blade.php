@extends('layouts.app')
@section('title', 'Thread — ' . $thread->subject->subject_name)
@section('content')
    <div class="card" style="max-width:640px;">
        <div class="card-header">
            <h2 style="margin:0; font-size:15px;">{{ $thread->subject->subject_name }} — {{ $thread->subject->teacher?->full_name ?? 'Teacher' }}</h2>
        </div>
        ...
        @if ($thread->status === 'Open')
            <div class="card-body" style="border-top:1px solid var(--line);">
                <form method="POST" action="{{ route('student.messages.reply', $thread) }}">
                    @csrf
                    <div class="field">
                        <textarea name="message" rows="3" placeholder="Write a message&hellip;" required></textarea>
                    </div>
